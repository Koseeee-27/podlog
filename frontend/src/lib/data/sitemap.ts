 ...
 * ### トークンローテーション時の挙動
 *
 * Next.js の fetch キャッシュキーは `Authorization` ヘッダーを含めてハッシュ化される
 * ため、Cloud Run / Netlify でトークンを差し替えると古いトークンの cache entry は
 * 即座にキー不一致でミスし、新トークン分は新しいキーで取得される。実害はゼロだが、
 * ローテーション直後は最初のリクエストで BE が叩かれる挙動になる。
 *
 * ## ファイル上限
 *
