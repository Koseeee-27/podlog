...
flowchart TD
    ...
    HOME -- "「番組を探す」押下（未ログイン時）" --> DISCOVER
    HOME -- "人気番組の「もっと見る」押下（未ログイン時）" --> DISCOVER
...
flowchart TD
    ...
    PUBLIC_CHECK -- "/profile/setup,<br>/settings,<br>/settings/profile" --> REDIRECT_LOGIN["/login へリダイレクト<br>（middleware）"]
...
- **タイムラインセクション**: グローバルタイムラインをメインで表示
...
- ヘッダーにロゴ「PodLog」を表示（クリックで `/` へ遷移）
- 「アカウントにログイン」見出し
- 「Google アカウントでログイン」ボタン
- 利用規約・プライバシーポリシーへの同意文言とリンク
...
- **アカウント情報カード**: ユーザー名、表示名、ログイン方法（Google）
- **「プロフィールを編集する」リンク** → `/settings/profile`
- **「ログアウト」ボタン**
...
| 要素     | SP          | PC               |
| ------ | ----------- | ---------------- |
| アクセス方法 | マイページ内からリンク | ヘッダーのドロップダウンから遷移 |
| レイアウト  | 全幅          | カード型で中央配置        |
...
- **デザイン（SP）**: TODO — `designs/record-sp.png`
- **デザイン（PC）**: TODO — `designs/record-pc.png`
...
| 要素     | SP              | PC                      |
| ------ | --------------- | ----------------------- |
| アクセス方法 | ボトムナビ「＋ 記録する」タブ | ヘッダーに「＋ 記録」ボタン or ナビリンク |
...
未ログイン
  ...
  │     → 書き込み操作時に /login へ誘導
  │
  └─→ /settings, /settings/profile, /record にアクセス → /login へリダイレクト
