...
| 13  | 記録する        | `/record`           | 必要  | main  | N | ログイン後限定 |
...
flowchart TD
    ...
    %% 記録する画面からの遷移
    RECORD -- "新着エピソードカード押下" --> EPISODE
    RECORD -- "検索結果からエピソード選択" --> EPISODE
    RECORD -- "未認証（middleware）" --> LOGIN
    RECORD -- "プロフィール未設定" --> SETUP
...
flowchart TD
    ...
    PUBLIC_CHECK -- "/profile/setup,<br>/settings,<br>/settings/profile,<br>/admin,<br>/record" --> REDIRECT_LOGIN["/login へリダイレクト<br>（middleware）"]
...
flowchart LR
    subgraph PC["PC: ヘッダーナビ"]
        ...
        PC_RECORD["「記録する」ボタン → /record<br>（プロフィール設定済み時のみ表示）"]
...
| 記録する  | Plus             | `/record`           | 聴いた番組の記録に特化した導線    |
| マイページ | User             | `/users/{username}` | 聴取履歴・評価サマリー・感想・プロフィール |
...
- 「記録する」タブは認証必須:
  - 未ログイン時: 押下すると middleware で `/login` にリダイレクト
  - プロフィール未設定時: 押下すると `/record` の Server Component でプロフィール確認後 `/profile/setup` にリダイレクト
...
| 「＋ 記録する」ボタン | 右（プロフィール設定済み） | `/record` へ遷移                  |
...
- **評価セクション（コンパクト）**:
  ...
  - 自分の評価入力 UI（星をクリックして 1〜5 選択 → 即時投稿）。投稿済みなら現在の星にハイライト
    - ※ podlog#407（P-6）時点では暫定で「星選択 → 『投稿する』ボタン」の submit 方式で実装している。即時投稿への切替は podlog#408 で対応予定
...
| 星をクリック（評価セクション）             | 評価を選択（1〜5）。即時に作成または更新（既存評価がなければ作成）<br>※ podlog#407 時点では暫定で submit ボタン押下方式。即時投稿への切替は podlog#408 |
...
| 聴取記録トグル中   | ボタンにスピナー表示           |
| 評価投稿成功     | 星にハイライト + 平均評価が更新（メッセージは出さず即時反映）<br>※ podlog#407 時点では暫定で「評価を投稿しました」トーストを表示。podlog#408 でメッセージ非表示に切替 |
...
| 聴取履歴・感想セクションの「もっと見る」押下 | 次のページを読み込み |
...
未ログイン
  ...
  │     → 書き込み操作時に /login へ誘導
  │
  └─→ /settings, /settings/profile, /admin, /record にアクセス → /login へリダイレクト
