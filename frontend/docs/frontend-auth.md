...
│    - props で認証状態を受け取る                            │
│    - signOut() でログアウト                                │
│    - Client API (lib/api/client.ts 経由) で追加データ取得  │
└────────────────┬─────────────────────────────────────────┘
                 │ リクエスト
┌────────────────▼─────────────────────────────────────────┐
...
> Next.js 16 で `middleware.ts` は `proxy.ts` にリネームされた。PodLog ではまだ `middleware.ts` を使用しており、本ドキュメントも `middleware.ts` を前提に書いている。将来のバージョンで移行する。

- **保護パス** (`/record`, `/profile/setup`, `/settings`, `/admin`) では `getUser()` で JWT を検証し、未認証なら `/login` にリダイレクトする
- **公開ページで認証 Cookie がある場合**のみ Supabase クライアントを作成し、`getSession()` でトークンリフレッシュを行う（`getUser()` は呼ばない）
- **公開ページで認証 Cookie がない場合**は Supabase クライアント生成自体をスキップする（最速パス）
...
- `import "server-only"` で Client Component からの import を防止
- GET 系（取得系）の DAL 関数は React `cache()` でラップし、同一リクエスト内の重複取得を防ぐ
- mutation（作成・更新・削除）の DAL 関数は `cache()` でラップせず、通常の `export async function` として実装する
