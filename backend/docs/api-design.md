...
| GET  | `/episodes/{id}`              | 不要  | エピソード詳細取得                 | 部分実装（同上） |
...
>
> **現在のステータス**: podlog#390（rating 切替）で旧 rating ベースの実装が削除され、`/timeline` は **一時的に未提供**（404）です。comment ベースの新実装は podlog#391 で復活します。リリース前のため FE 側のタイムラインは P-8 までは空表示で許容します。
