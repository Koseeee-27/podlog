...
| GET  | `/podcasts/popular`     | 不要  | 人気番組一覧（評価件数順）       | 実装済み |
...
1ユーザーにつき1エピソード1評価。既に投稿済みの場合は 409 を返す（更新は `PUT /episodes/{id}/ratings/mine` を使う）。
