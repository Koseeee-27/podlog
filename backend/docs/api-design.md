...
- **既知の制約**: 上記 2 の「既存番組を検索結果に追加するだけ」のケースでも、現行実装では DB の集計値を読み直していないため `average_rating` / `total_reviews` / `favorite_count` が `0` で返る（DB の実値を使っていない）。レビューやお気に入りが付いた既存番組でもこの経路では集計値が 0 になる点に注意。実装修正は podlog#351 を参照
