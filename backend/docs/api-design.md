...
| GET  | `/users/{username}/ratings/stats`     | 不要  | ユーザーの評価統計サマリー（公開） | 実装済み |
| GET  | `/users/{username}/comments`          | 不要  | ユーザーの感想一覧（公開）      | 実装済み |
...
| メソッド | パス                      | 認証  | 説明                  | 状態   |
| ---- | ----------------------- | --- | ------------------- | ---- |
| GET  | `/podcasts/search`      | 不要  | アプリ内 DB の番組をキーワード検索 | 部分実装（`total_ratings` 反映済み・`total_comments` は未対応） |
| GET  | `/podcasts/popular`     | 不要  | 人気番組一覧（評価件数順）       | 部分実装（同上） |
| POST | `/podcasts/request`     | 必要  | 番組の追加リクエスト          | 実装済み |
| GET  | `/podcasts/{id}`        | 不要  | ポッドキャスト詳細取得         | 部分実装（同上） |
...
| メソッド | パス                            | 認証  | 説明                        | 状態   |
| ---- | ----------------------------- | --- | ------------------------- | ---- |
| GET  | `/podcasts/{id}/episodes`     | 不要  | エピソード一覧取得                 | 部分実装（`total_ratings` 反映済み・`total_comments` は未対応） |
...
| メソッド   | パス                            | 認証  | 説明                  | 状態   |
| ------ | ----------------------------- | --- | ------------------- | ---- |
| POST   | `/episodes/{id}/ratings`      | 必要  | 評価投稿                | 実装済み |
| GET    | `/episodes/{id}/ratings/mine` | 必要  | 自分の評価取得             | 実装済み |
| PUT    | `/episodes/{id}/ratings/mine` | 必要  | 自分の評価更新             | 実装済み |
| DELETE | `/episodes/{id}/ratings/mine` | 必要  | 自分の評価削除             | 実装済み |
| GET    | `/episodes/{id}/ratings`      | 不要  | エピソードの評価集計（平均・件数・分布） | 実装済み |
| GET    | `/users/me/ratings`           | 必要  | 自分の評価一覧             | 実装済み |
...
| メソッド   | パス                       | 認証  | 説明              | 状態   |
| ------ | ------------------------ | --- | --------------- | ---- |
| POST   | `/episodes/{id}/comments` | 必要  | 感想投稿（複数可）       | 実装済み |
| GET    | `/episodes/{id}/comments` | 不要  | エピソードの感想一覧      | 実装済み |
| PUT    | `/comments/{id}`         | 必要  | 感想更新（投稿者本人のみ）  | 実装済み |
| DELETE | `/comments/{id}`         | 必要  | 感想削除（投稿者本人のみ）  | 実装済み |
| GET    | `/users/me/comments`     | 必要  | 自分の感想一覧         | 実装済み |
...
| メソッド | パス          | 認証  | 説明                 | 状態 |
| ---- | ----------- | --- | ------------------ | -- |
| GET  | `/timeline` | 不要  | 最新の感想のタイムライン        | 実装済み |
...
| 日付         | 変更内容                                                 |
| ---------- | ---------------------------------------------------- |
| 2026-05-01 | 評価/感想分離の BE 実装第2弾（podlog#391 / 親 Issue: podlog-workspace#59）。`comments` 参照の Go コード一式（model / repository / usecase / handler）を新設し、エンドポイント `POST /episodes/{id}/comments`, `GET /episodes/{id}/comments`, `PUT /comments/{id}`, `DELETE /comments/{id}`, `GET /users/me/comments`, `GET /users/{username}/comments` を実装。`PUT/DELETE /comments/{id}` は所有者チェック失敗時に **403**、リソース不存在時に **404** を返すよう区別（仕様書準拠）。エピソード詳細レスポンスに `total_comments` フィールドを追加（軽量 COUNT クエリで取得）。podlog#390 で一時撤去していた `GET /timeline` を comment ベースで復活。番組単位の集計（`/podcasts/{id}` および検索・一覧の `total_comments`）と番組詳細レスポンスへの追加は本 PR のスコープ外で、別 Issue でフォローアップ予定 |
| 2026-05-01 | 評価/感想分離の BE 実装第1弾（podlog#390 / 親 Issue: podlog-workspace#59）。`reviews` 参照の Go コード（model / repository / usecase / handler / router / DI）を全廃し、`ratings` 系（`POST/GET/PUT/DELETE /episodes/{id}/ratings(/mine)`, `GET /users/me/ratings`, `GET /users/{username}/ratings/stats`, `GET /podcasts/{id}/rating`）に置き換え。エピソード詳細・番組詳細・検索・人気番組のレスポンスフィールド `total_reviews`（当時の表記。現: `total_ratings`）を `total_ratings` にリネーム実装。`GET /episodes/{id}/ratings` は集計 API（平均・総件数・1〜5 の星別ヒストグラム）として新設。`GET /users/{username}/ratings/stats` は集計サマリー API として新設。`/timeline` は本 PR で一時撤去（comment ベース実装は podlog#391 で復活）。最終コミットで `000012_drop_reviews` マイグレーションを追加し reviews テーブルを削除（DROP は Go コード置き換え後の最後に流すルールに準拠）。`total_comments` フィールドの追加は podlog#391 で対応するため、契約フィールドが部分的にしか揃っていない既存エンドポイントは状態カラムを「部分実装」に変更 |
