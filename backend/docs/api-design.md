...
RSS フィードがない番組（Spotify 独占等）の手動登録用。MVP では認証済みユーザーなら誰でも使用可能。

| メソッド | パス                                | 認証  | 説明            | 状態   |
| ---- | --------------------------------- | --- | ------------- | ---- |
| POST | `/admin/podcasts`                 | 必要  | 番組の手動登録       | 実装済み |
| POST | `/admin/podcasts/{id}/episodes`   | 必要  | エピソードの手動登録    | 実装済み |
...
RSS フィードがない番組（Spotify 独占等）を手動で登録する。`feed_url` なしで登録可能。MVP では認証済みユーザーなら誰でも使用可能。
