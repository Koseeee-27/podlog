...
- **識別子**: `echo.Context.RealIP()` で取得する IP アドレス（Cloud Run 前段の Google Front End が付与する `X-Forwarded-For` の先頭要素）
...
クライアントは `Retry-After` ヘッダの秒数（60 秒）待機してから再試行すること。
...
- ミドルウェアの適用順序は **レート制限 → タイムアウト → JWT 検証** の順。拒否されたリクエストで不要なコスト（タイムアウトコンテキストの生成・JWT 検証）を払わないようにしている
