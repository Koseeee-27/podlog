...
CREATE UNIQUE INDEX idx_listening_records_user_episode
    ON listening_records (user_id, episode_id);

-- ユーザーの聴取履歴一覧（新しい順）取得用
CREATE INDEX idx_listening_records_user_id_created_at
    ON listening_records (user_id, created_at DESC);

-- エピソードの聴取数カウント用
CREATE INDEX idx_listening_records_episode_id
