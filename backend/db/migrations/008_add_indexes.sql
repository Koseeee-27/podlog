...
CREATE INDEX idx_podcasts_title_author_trgm ON podcasts USING gin (
    title gin_trgm_ops,
    author gin_trgm_ops
);

-- feed_url の重複防止用（番組の重複登録を防ぐ）
CREATE UNIQUE INDEX idx_podcasts_feed_url ON podcasts (feed_url) WHERE feed_url IS NOT NULL;
...
CREATE UNIQUE INDEX idx_episodes_itunes_track_id ON episodes (itunes_track_id) WHERE itunes_track_id IS NOT NULL;
...
CREATE UNIQUE INDEX idx_podcasts_itunes_id ON podcasts (itunes_id) WHERE itunes_id IS NOT NULL;
