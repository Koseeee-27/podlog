...
| バックエンド | Go / Echo | フロントエンドとは別の言語で開発してみたく採用。軽量・高速で、静的型付けと書きやすい並行処理により保守性も確保できる |
...
| ホスティング | Netlify (フロントエンド) / Cloud Run (バックエンド) | フロントは静的配信に強い Netlify、API はコンテナをそのままデプロイでき自動スケールする Cloud Run |
